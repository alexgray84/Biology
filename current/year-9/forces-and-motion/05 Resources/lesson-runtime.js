...
(() => {
  ...
  const list=items=>`<ol class="bio-list">${items.map(s=>`<li>${s}</li>`).join('')}</ol>`;
  ...
  function content(p) {
    const s=state();
    if(p.type==='title')return `<div class="title-layout"><p class="screen-kicker">Year 9 Science · ${esc(lesson.unit)} · Lesson ${lesson.unitLesson}</p><h1 class="screen-heading" tabindex="-1">${lesson.title}</h1><p class="title-question">${lesson.question}</p><div class="title-path">${lesson.path.map(x=>`<span>${x}</span>`).join('')}</div></div>`;
    ...
    if(p.type==='outcome')return `${heading}<ol class="outcome-list">${lesson.outcomes.map(o=>`<li><div><strong>${o[0]}</strong><span class="spec-ref">Year 9 curriculum · ${o[1]}</span></div></li>`).join('')}</ol>`;
    ...
    if(p.type==='visual')return `${heading}<div class="science-visual">${p.graphHref?`<iframe class="graph-embed" data-phase="${p.id}" title="Step-by-step distance–time graph model" src="${esc(p.graphHref)}"></iframe>`:p.image?`<figure class="journey-figure"><img src="${esc(p.image.src)}" alt="${esc(p.image.alt)}"><figcaption>${p.image.caption}</figcaption></figure>`:p.rows?`<table class="science-table">${p.rows.map((r,i)=>`<tr>${r.map(c=>i===0?`<th>${c}</th>`:`<td>${c}</td>`).join('')}</tr>`).join('')}</table>`:''}<div class="visual-instruction"><div class="bio-step"><span class="step-number">STEP ${s.step+1} OF ${p.steps.length}</span><p>${p.steps[s.step]}</p></div><div class="bio-controls">${button('← Back','step',-1,s.step===0?'disabled':'')}${button('Next step →','step',1,s.step===p.steps.length-1?'disabled':'')}</div></div></div>`;
    if(p.type==='eye')return `${heading}<iframe class="eye-embed" title="Rotatable labelled eye" src="Resources/eye-model.html?embed"></iframe><p class="bio-task">${p.task}</p>`;
  ...
  function render(focusHeading=false) {
    ...
    const keepEye=p.graphHref&&$('screen').querySelector('.graph-embed')?.dataset.phase===p.id;
    if(keepEye){$('screen').querySelector('.review-access-row').outerHTML=review;const template=document.createElement('template');template.innerHTML=content(p);$('screen').querySelector('.visual-instruction').replaceWith(template.content.querySelector('.visual-instruction'));}else $('screen').innerHTML=review+content(p)+(p.resources?.length?`<div class="resource-links">${p.resources.map(r=>`<a href="${esc(r.href)}" target="_blank" rel="noopener">${r.label} ↗</a>`).join('')}</div>`:'');
