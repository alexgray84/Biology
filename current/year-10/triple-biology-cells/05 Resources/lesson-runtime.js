...
(() => {
  ...
  function content(p) {
    ...
    if(p.type==='model')return `${heading}<div class="bio-layout">${figure(p.diagram,p.caption)}<div><div class="bio-step"><span class="step-number">STEP ${s.step+1} OF ${p.steps.length}</span><p>${p.steps[s.step]}</p></div><div class="bio-controls">${button('← Back','step',-1,s.step===0?'disabled':'')}${button('Next step →','step',1,s.step===p.steps.length-1?'disabled':'')}</div></div></div>${p.task?`<p class="bio-task">${p.task}</p>`:''}`;
