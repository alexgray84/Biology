(() => {
  ...
  // Week 2 learning overlay: one short, contextual route at a time. It layers
  // over the canvas rather than reflowing it, so the board view stays full size.
  const learningOverlay = document.querySelector('.learning-overlay');
  const learningToolsButton = document.querySelector('[data-learning-tools]');
  const learningToolBody = document.querySelector('.learning-tool-body');
  const learningTools = JSON.parse(document.querySelector('#learning-tools-data')?.textContent || '{}');
  const learningLabels = { start: 'Get started', model: 'See the model', stretch: 'Stretch it' };
  function renderLearningTools(key) {
    if (!learningOverlay || !learningToolBody) return;
    const currentLabel = slides[current]?.dataset.label || '';
    const routes = learningTools.slides?.[currentLabel] || learningTools.default || {};
    const items = (routes[key] || []).slice(0, 3);
    document.querySelectorAll('.learning-tool-tab').forEach(tab => tab.setAttribute('aria-selected', String(tab.dataset.learningTab === key)));
    learningToolBody.innerHTML = `<p class="learning-tool-kicker">${learningLabels[key]}</p><h2>${currentLabel || 'This screen'}</h2><ol>${items.map(item => `<li>${item}</li>`).join('')}</ol>`;
  }
  function setLearningOverlay(open) {
    if (!learningOverlay || !learningToolsButton) return;
    learningOverlay.hidden = !open;
    learningToolsButton.setAttribute('aria-expanded', String(open));
    if (open) renderLearningTools(document.querySelector('.learning-tool-tab[aria-selected="true"]')?.dataset.learningTab || 'start');
  }
  learningToolsButton?.addEventListener('click', () => setLearningOverlay(learningOverlay.hidden));
  document.querySelector('[data-close-learning-tools]')?.addEventListener('click', () => setLearningOverlay(false));
  document.querySelectorAll('.learning-tool-tab').forEach(tab => tab.addEventListener('click', () => renderLearningTools(tab.dataset.learningTab)));
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && learningOverlay && !learningOverlay.hidden) setLearningOverlay(false);
  });
